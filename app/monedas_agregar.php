...
//if (bd_verificar_privilegios('monedas_agregar.php',$_SESSION['usuario']['nivel_id'])!='CONCEDER')
//{
...
$f = new FormHandler('monedas',NULL,'role="form"');
$f -> useTable( false );
...
$f->textField( $f_meta['abreviacion']['etiqueta'], 'abreviacion', FH_STRING, 50, 255 );
...
#$f->setValue('nombre', '');
#$f->setValue('abreviacion', '');

$f -> submitButton('Continuar','botonSubmit');
$f -> borderStop();
$f -> onCorrect('proc_monedas_agregar');

function proc_monedas_agregar($d)
{
    bd_monedas_agregar($d);
...
$smarty -> display("monedas_agregar.html");
